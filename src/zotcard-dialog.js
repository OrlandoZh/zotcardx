...
Zotero.ZotCard.Dialogs = Object.assign(Zotero.ZotCard.Dialogs, {
	openWindow(url, name, features, args) {
		if (typeof Services !== 'undefined' && Services.ww) {
			return Services.ww.openWindow(null, url, name, features, args);
		}
		return Zotero.getMainWindow().openDialog(url, name, features, args);
	},
	...
	openCardManager(items, filters) {
		...
		let win = this.openWindow('chrome://zotcard/content/cardmanager/card-manager.html', 'card-manager', 'chrome,menubar=no,toolbar=no,dialog=no,resizable,centerscreen,height=' + Zotero.getMainWindow().screen.availHeight + ',width=' + Zotero.getMainWindow().screen.availWidth, io);
		win.focus();
	},
	...
	openCardManagerTab(items, filters) {
		...
		let iframe = doc.createXULElement ? doc.createXULElement('browser') : doc.createElement('browser');
	...
	openMultiEditManager(noteIDs) {
		...
		let win = this.openWindow('chrome://zotcard/content/multieditmanager/multi-edit-manager.html', 'multi-edit-manager', `chrome,menubar=no,toolbar=no,dialog=no,height=200,width=220,left=${Zotero.getMainWindow().screen.availWidth - 205},top=${Zotero.getMainWindow().screen.availWidth - 225}`);
		win.focus();
	},
	...
	openCardViewer(items) {
		...
		let windowCardViewer = this.openWindow('chrome://zotcard/content/cardviewer/card-viewer.html', 'card-viewer', 'chrome,menubar=no,toolbar=no,dialog=no,resizable,centerscreen,height=' + Zotero.getMainWindow().screen.availHeight + ',width=' + Zotero.getMainWindow().screen.availWidth, io);
	...
	openCardReplace(items, cards) {
		...
		let win = this.openWindow('chrome://zotcard/content/cardreplace/card-replace.html', 'card-replace', 'chrome,menubar=no,toolbar=no,dialog=no,resizable,centerscreen,height=' + Zotero.getMainWindow().screen.availHeight + ',width=' + Zotero.getMainWindow().screen.availWidth, io);
		win.focus();
	},
	...
	openCardImageCompression(items) {
		...
		let windowCardViewer = this.openWindow('chrome://zotcard/content/cardimagecompression/card-image-compression.html', 'card-image-compression', 'chrome,menubar=no,toolbar=no,dialog=no,resizable,centerscreen,height=' + Zotero.getMainWindow().screen.availHeight + ',width=' + Zotero.getMainWindow().screen.availWidth, io);
		windowCardViewer.focus();
	},
});
