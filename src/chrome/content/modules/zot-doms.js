...
Zotero.ZotCard.Doms = Object.assign(Zotero.ZotCard.Doms, {
  ...
  createXULElement(document, tag, { id, attrs, props, parent, after, before, childs, command, onclick }) {
    ...
    let element = document.createXULElement ? document.createXULElement(tag) : document.createElement(tag);
    if (id) {
      element.id = id;
    }
    if (attrs) {
  ...
  getMainWindowPaneAddChildNotePopup() {
    let doc = Zotero.getMainWindow().document;
    return doc.getElementById('context-pane-add-child-note-button-popup')
      || doc.querySelector('.context-pane-add-child-note-button-popup');
  },
