...
Zotero.ZotCard.Preferences = {
	...
	_getString: function(name, params) {
		if (!this._l10n) {
			return name;
		}
		return params ? this._l10n.formatValueSync(name, params) : this._l10n.formatValueSync(name);
	},

	_createHtmlElement: function(tag) {
		return document.createElementNS('http://www.w3.org/1999/xhtml', tag);
	},

	_mountTempElement: function(element) {
	...
	backup: function() {
		let { now, backup } = this._buildBackupData();
		let blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' });
		let url = URL.createObjectURL(blob);
		let link = this._createHtmlElement('a');
		let unmount = this._mountTempElement(link);
		link.href = url;
		link.download = now + '.zotcard';
		link.click();
		unmount();
		URL.revokeObjectURL(url);
		Zotero.ZotCard.Messages.success(window, this._getString('zotero-zotcard-preferences-backup-successful'));
	},

	restore: function() {
		let input = this._createHtmlElement('input');
		input.type = 'file';
		input.accept = '.zotcard';
		input.style.display = 'none';
		let unmount = this._mountTempElement(input);
				...
				if (content) {
					try {
						...
						if (backup.last_updated) {
							...
							Zotero.ZotCard.Messages.success(window, this._getString('zotero-zotcard-preferences-restore-successful', {last_updated: backup.last_updated}));
						} else {
							Zotero.ZotCard.Messages.warning(window, this._getString('zotero-zotcard-preferences-restore-error-file'));
						}
					} catch (e) {
	...
	reset: function() {
		if (Zotero.ZotCard.Messages.confirm(window, this._getString('zotero-zotcard-preferences-reset-confirm'))) {
			Zotero.ZotCard.Consts.defCardTypes.forEach(type => {
				Zotero.ZotCard.Prefs.clear(type);
				Zotero.ZotCard.Prefs.clear(`${type}.label`);
				Zotero.ZotCard.Prefs.clear(`${type}.visible`);
			...
			Zotero.ZotCard.Messages.success(window, this._getString('zotero-zotcard-preferences-reset-successful'));
		}
	}
}
