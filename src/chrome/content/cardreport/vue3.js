       ...
       let iframe = document.querySelector('#editor-view');
        iframe.addEventListener('DOMContentLoaded', (event) => {
          ...
          iframe.contentWindow.addEventListener('drop', (event) => {
            ...
            try {
              iframe.contentWindow.wrappedJSObject.droppedData = Components.utils.cloneInto(droppedData, iframe.contentWindow);
            } catch (e) {
              iframe.contentWindow.wrappedJSObject.droppedData = droppedData;
            }
          }, true);
        });
