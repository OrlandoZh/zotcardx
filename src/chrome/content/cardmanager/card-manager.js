...
// Try to get data from window.arguments (Services.ww.openWindow mode)
try {
  ...
  Zotero.logError('card-manager window arguments init failed: ' + e);
}

// Fallback: try tab mode (Zotero 7 tabs)
  ...
  try {
    ...
    if (tabID) {
      ...
      if (tab && tab.tab && tab.tab.type === 'library' && tab.tab.id.startsWith('card-manager-')) {
        dataIn = tab.tab.data ? tab.tab.data.dataIn || [] : [];
        _filters = tab.tab.data ? tab.tab.data.filters || {} : {};
        from = 'tab';
      }
    }
  } catch (e) {
    Zotero.logError('card-manager tab init failed: ' + e);
  }
}

const parentIDs = (dataIn && dataIn.length) ? Zotero.ZotCard.Cards.parseParentIDs(dataIn) : [];
...
async function _initCardManager() {
  if (_initCardManager._done) return;
  _initCardManager._done = true;
  const _pagesize = 15;
  ...
  try {
    ...
    setup() {
            ...
            if (level === 0) {
              ...
              switch (data.extras.type) {
                case Zotero.ZotCard.Consts.cardManagerType.library:
                  ...
                  // load items
                  let itemsAll = await Zotero.Items.getAll(data.extras.id, true, false);
                  let rets = Array.from(itemsAll).filter(e => (e.isRegularItem() || e.isNote()) && e.getCollections().length === 0);
            ...
            if (level === 0) {
              ...
              switch (data.extras.type) {
                case 'library':
                  ...
                  // load items
                  let itemsAll2 = await Zotero.Items.getAll(data.extras.id, true, false);
                  let rets = Array.from(itemsAll2).filter(e => (e.isRegularItem() || e.isNote()) && e.getCollections().length === 0);
                  nodes.push(...rets.map(i => {
                    return {
                      label: i.getDisplayTitle(),
                      value: (i.isRegularItem() ? Zotero.ZotCard.Consts.cardManagerType.item : i.itemType) + '-' + i.id,
      ...
      const hasMoreToLoad = computed(() => moreLoadeds.value < total.value);
      const cardViewerPopoverEmpty = computed(() => renders.cardViewerPopover.total <= 0);
    ...
    Zotero.logError('card-manager init failed: ' + e);
    let errDiv = document.createElement('div');
    errDiv.style.cssText = 'color:red;padding:20px;background:white;margin:10px;font-size:14px;font-family:monospace;white-space:pre-wrap;';
...
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', _initCardManager, { once: true });
} else {
  _initCardManager();
}
