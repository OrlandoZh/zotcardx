...
Zotero.ZotCard = Object.assign(Zotero.ZotCard, {
	...
	initWindow(window) {
		...
		let ftlLink = window.document.getElementById('zotcard-localization');
		if (!ftlLink) {
			ftlLink = window.document.createElement('link');
			ftlLink.id = 'zotcard-localization';
			ftlLink.setAttribute('rel', 'localization');
			ftlLink.setAttribute('href', 'zotcard.ftl');
			window.document.documentElement.appendChild(ftlLink);
			this.storeAddedElement(ftlLink);
		}
	...
	createNoteToolbar() {
		...
		let template = editor_view_middle.ownerDocument.createElement('template');
		template.innerHTML = `<div id="zotero-card-editor" class="dropdown text-dropdown">
		...
		let divOpenCardEditor = template.content.querySelector('div');
			...
			command: () => {
				...
				if (win) {
					...
					if (Zotero.ZotCard.Objects.isNoEmptyArray(cardManagerTabs)) {
						...
						Zotero.ZotCard.Dialogs.openCardManager([]);
					}
				}
			},
	...
	createItemMenu() {
		...
		menuitem = Zotero.ZotCard.Doms.createMainWindowXULElement('menuitem', {
			...
			command: () => {
				let io = {};
				Zotero.ZotCard.Dialogs.openWindow('chrome://zotcard/content/batchnewcard/batchnewcard.html', 'batchnewcard', 'chrome,modal,centerscreen,scrollbars', io);
		...
		let separatorSettings = Zotero.ZotCard.Doms.createMainWindowXULMenuSeparator({
			id: `${root}-zotcard-menu-menupopup-separator-settings`,
			parent: mnupopupZotCard
		});
		separatorSettings.hidden = false;

		let cardSettingsMenu = Zotero.ZotCard.Doms.createMainWindowXULElement('menuitem', {
			id: `${root}-zotcard-card-settings-menuitem`,
			command: () => {
				Zotero.openInViewer('chrome://zotcard/content/preferences/zotcard-preferences.html');
			},
			parent: mnupopupZotCard
		});
		cardSettingsMenu.setAttribute('label', Zotero.ZotCard.L10ns.getString('zotcard-preferences'));
		cardSettingsMenu.hidden = false;
		zotcardMenu.disabled = false;
	...
	createPaneMenu() {
		...
		let elPanePopup = Zotero.ZotCard.Doms.getMainWindowPaneAddChildNotePopup();
		if (!elPanePopup) {
			Zotero.ZotCard.Logger.log('context pane add-child-note popup not found.');
			return;
		}
		...
		elPanePopup.querySelectorAll('.custom-card').forEach(e => {
			e.hidden = true;
		})
		hasMenuitem = 0;
		let quantity = Zotero.ZotCard.Prefs.get('card_quantity', Zotero.ZotCard.Consts.card_quantity);
	...
	createStandaloneMenu() {
		...
		let settingsMenuitem = Zotero.ZotCard.Doms.createMainWindowXULElement('menuitem', {
			id: `${root}-zotcard-settings`,
			command: () => {
				Zotero.openInViewer('chrome://zotcard/content/preferences/zotcard-preferences.html');
			},
			parent: zotero_tb_note_add_menupopup
		});
		settingsMenuitem.setAttribute('label', Zotero.ZotCard.L10ns.getString('zotcard-preferences'));
		this.storeAddedElement(settingsMenuitem);
	},
	...
	registerEvent() {
		...
		this._cardQuantityObserverID = Zotero.Prefs.registerObserver('zotcard.card_quantity', function () {
	...
	shutdown() {
		...
		if (this._cardQuantityObserverID) {
			Zotero.Prefs.unregisterObserver(this._cardQuantityObserverID);
			this._cardQuantityObserverID = null;
		}
